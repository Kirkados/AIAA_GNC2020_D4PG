...
class Environment:
    ...
    def __init__(self):
        ...
        ##################################
        self.TOTAL_STATE_SIZE         = 6 # [x, y, theta, desired_x_error, desired_y_error, desired_theta_error]
        ...
        self.TARGET_COLLISION_PENALTY  = 5           # [rewards/second] penalty given for colliding with target  

        # Additional properties
        self.PHASE_1_TIME             = 45 # [s] the time to automatically switch from phase 0 to phase 1
        self.DOCKING_TOO_FAST_PENALTY = 0 # [rewards/s] penalty for docking too quickly
        self.MAX_DOCKING_SPEED        = [0.02, 0.02, 10]
        self.TARGET_ANGULAR_VELOCITY  =  0.0698 #[rad/s] constant target angular velocity
...
#            print("Entered docking phase!")
    ...
    def reward_function(self, action):
        ...
        if self.REWARD_TYPE:
            ...
            current_position_reward = -np.abs((target_location - self.state[:self.POSITION_STATE_LENGTH])*self.REWARD_WEIGHTING)* self.TARGET_REWARD
            #print(reward_old, current_position_reward)
            #raise SystemExit
        else:
            # Exponential reward
            current_position_reward = np.exp(-np.sum(np.absolute(target_location - self.state[:self.POSITION_STATE_LENGTH])*self.REWARD_WEIGHTING)) * self.TARGET_REWARD
        ...
        if np.sum(np.absolute(self.state[:self.POSITION_STATE_LENGTH] - target_location)) < 0.01:
